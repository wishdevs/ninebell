...
/**
 * 한국 공휴일 — `@hyunbinseo/holidays-kr`(우주항공청 월력요항 기반, 2018~2027 정적 내장,
 * 연 단위 버전업으로 다음 해 데이터 추가)가 단일 소스다(사용자 지정 2026-08-21).
 * 정적 임포트라 런타임 네트워크 의존이 없고, 대체·임시공휴일·선거일까지 수록된다.
 * 수록 범위 밖 연도는 종전처럼 주말 보정만 적용된다 — 연도 확장은 의존성 업데이트로.
 *
 * ⚠ 종전 수기 테이블의 오류 2건이 이 교체로 바로잡혔다: 2026-09-28 은 추석 대체공휴일이
 *   아니고(설·추석 연휴는 일요일 겹침만 대체 대상 — 2026 연휴는 목·금·토), 노동절(05-01)·
 *   제헌절(07-17)은 공휴일인데 빠져 있었다.
 */
import * as holidayYears from '@hyunbinseo/holidays-kr/all';

const HOLIDAYS: ReadonlySet<string> = new Set(
  Object.values(holidayYears).flatMap((year) => Object.keys(year)),
);
