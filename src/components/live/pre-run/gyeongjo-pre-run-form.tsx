...
// 합리적 상한(오타·단위 실수 방지) — 경조금 정액(공급가액) 1억 원.
...
/**
 * 최종 공급가액 = 근속 1년 미만이면 정액 × 0.5(원 단위 반올림), 아니면 정액 그대로(D10-a).
 * 양수 정수라 Math.round(사사오입, half-up)는 백엔드 supply_amount 의 Decimal ROUND_HALF_UP 과
 * 결과가 완전히 같다(예 100001 → 50001). baseAmount 는 안전정수 범위라 부동소수 오차 없음.
 */
function supplyAmount(baseAmount: number, under1Year: boolean): number {
...
function seedFromParams(initial: Record<string, unknown> | undefined): {
  ...
  const gj = initial?.gyeongjo as Record<string, unknown> | undefined;
  if (!gj || typeof gj !== 'object') return {};
  const project = (gj.project ?? {}) as { code?: string; name?: string };
  return {
    evidenceDate: typeof gj.evidenceDate === 'string' ? gj.evidenceDate : undefined,
    baseAmount: gj.baseAmount != null ? String(gj.baseAmount) : undefined,
    under1Year: typeof gj.under1Year === 'boolean' ? gj.under1Year : undefined,
    project: project.code ? { code: project.code, name: project.name ?? '' } : undefined,
  };
}

export function GyeongjoPreRunForm({ disabled, initialParams, onStart }: PreRunFormProps) {
  ...
  const [under1Year, setUnder1Year] = useState<boolean>(() => seed.under1Year ?? false);
  ...
  // 최종 공급가액 미리보기 — 백엔드 supply_amount 와 반드시 동일 결과(근속<1년이면 반값).
  const supplyPreview = amountValid ? supplyAmount(Number(baseAmount), under1Year) : null;

  const submit = () => {
    if (!canSubmit) return;
    // 백엔드 parse_gyeongjo_params 계약: params["gyeongjo"] = 단건 객체.
  ...
  return (
    <SectionCard
      caption="실행 전 입력"
      title="경조금신청서 결의서"
      description="증빙일(=회계일)·경조금 정액(총액)·프로젝트만 입력하면 무개입으로 채워 저장합니다. 거래처·적요는 작성자 본인 이름으로, 예산단위(복리후생비-경조)는 소속 팀 비용구분으로 자동 지정됩니다. 경조사 1건 = 결의서 1장(단건)."
      density="comfortable"
    >
      ...
      <div className="border-warning/40 bg-warning/5 flex gap-2 rounded-[var(--radius-md)] border p-3">
        ...
        <div className="text-xs">
          ...
          <p className="text-foreground-tertiary mt-1 leading-relaxed">
            값(증빙일·정액·근속 여부·프로젝트)을 다시 확인하세요. 저장 후 <b>상신(결재)</b>은
            자동화하지 않습니다 — ERP 에서 직접 진행하세요.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-5 sm:max-w-md">
        ...
        <FormField
          ...
          hint={amountValid ? undefined : '규정표가 아닌 지급할 총액을 직접 입력합니다.'}
        >
          <Input
            id="gj-base-amount"
            type="number"
        ...
        {/* 프로젝트 */}
        <FormField id="gj-project" label="프로젝트" required>
          <CatalogCombobox
            value={project}
            placeholder="프로젝트 선택"
      ...
      {/* 최종 공급가액 미리보기 — 백엔드 계산과 동일 결과(실시간) */}
      <div className="border-border/60 bg-muted/30 flex flex-wrap items-center justify-between gap-3 rounded-[var(--radius-md)] border px-4 py-3">
        <div className="flex flex-col gap-0.5">
          ...
          {under1Year && supplyPreview != null ? (
            <span className="text-foreground-tertiary text-xs">
              정액 {Number(baseAmount).toLocaleString('ko-KR')}원의 50%
            </span>
          ) : null}
        </div>
