...
/**
 * 증빙유형 질문 섹션 — 질문 전부를 한 화면에 행으로 눕힌 폼(시뮬레이션 1단계 승격).
 *
 * 첫 화면은 증빙유형 카드 포털(사용자 확정 2026-08-06) — 숙련자는 카드 1클릭, 모르면
 * '질문으로 찾기'로 폼 진입. 위저드 없이 그 자리에서 다른 선택지를 누른다(2026-08-04 확정).
 * 활성 표시는 ring 이 아니라 **테두리+배경**만 쓴다(포커스링 금지 — 2026-08-04 사용자 확정).
 *
 * 무효화 규칙: 발행 전/후를 바꾸면 뒤 답 전부 초기화, 분할을 켜면 불공 해제.
 */
export function QuestionsSection({ value, onChange, disabled }: QuestionsSectionProps) {
  ...
  const pickIssue = (issue: IssueState) => {
    ...
    // 발행 후는 기간을 계약 기본값(이번 달 1일~오늘)으로 미리 채운다.
  ...
  // 답이 이미 채워진 채 열리면(기본 03 조합·복원) 폼으로 바로 연다.
  ...
  if (entryMode === 'portal') {
    return (
      <div className="flex flex-col gap-4">
        ...
        <div>
          <Button variant="secondary" disabled={disabled} onClick={() => setEntryMode('form')}>
            잘 모르겠어요 — 질문으로 찾기
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <button
        ...
        className="text-foreground-tertiary hover:text-foreground self-start text-[length:var(--text-caption)] transition-colors outline-none disabled:opacity-50 pointer-coarse:-mx-2 pointer-coarse:min-h-11 pointer-coarse:px-2"
      >
        증빙유형 코드로 바로 선택
      </button>
        ...
        <QuestionRow label={QUESTION_SHORT.issue}>
          ...
          {value.issue ? (
            <p className="text-foreground-tertiary text-[length:var(--text-body-sm)] leading-snug">
              {value.issue === 'before'
                ? '아직 발행되지 않아 거래처·공급가액을 아래에서 직접 입력합니다. 발행 전은 비용분할이 불가합니다.'
                : '이미 발행되어 조회됩니다 — 실행하면 아래 기간으로 (세금)계산서를 조회하고, 처리할 행은 실행 중 개입 화면에서 고릅니다.'}
            </p>
          ) : null}
        </QuestionRow>
          ...
          <QuestionRow
            ...
            help={
              <HelpTip label="세금계산서일 기간 설명">
                실행 중 이 기간에 발행된 (세금)계산서만 조회되어 개입 화면 목록에 나타납니다.
                기본값은 이번 달 1일부터 오늘까지입니다.
              </HelpTip>
            }
          >
          ...
          <QuestionRow
            ...
            help={
              <HelpTip label="비용분할 설명">
                한 장의 세금계산서를 프로젝트·비용센터별로 나눠 다는 것입니다. 분할하면 증빙유형이
                원증빙 계열(과세 11 · 비과세 13)로 바뀝니다.
              </HelpTip>
            }
          >
            <div className="flex flex-wrap gap-2">
      ...
      {/* 하단 한 줄 — 지금 답이 어떤 증빙유형 코드가 되는지(증빙 코드는 서버가 같은 규칙으로 도출). */}
      <div className="border-border-subtle bg-muted/40 flex flex-wrap items-center gap-x-3 gap-y-1.5 rounded-[var(--radius-md)] border px-3.5 py-2.5">
        ...
        {ev ? (
          ...
          <span className="text-foreground-secondary text-[length:var(--text-body-sm)]">
            {complete ? '' : `${pending}를 고르면 증빙유형이 정해집니다.`}
          </span>
        )}
      </div>
    </div>
...
function PortalCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'flex cursor-pointer items-center gap-2.5 rounded-[var(--radius-md)] border px-3.5 py-2.5 text-left transition-colors outline-none disabled:cursor-not-allowed disabled:opacity-50 pointer-coarse:min-h-11',
        active
...
/**
 * 선택지 pill — 활성 표시는 테두리+배경만(ring 금지 — 2026-08-04 사용자 확정).
 * 규격은 같은 줄의 폼 컨트롤(DatePicker·Button md)과 맞춘 h-10, 터치 기기는 44px(h-11).
 */
function ChoicePill({
  label,
  active,
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'h-10 cursor-pointer rounded-[var(--radius-sm)] border px-4 text-[length:var(--text-body)] font-medium transition-colors outline-none disabled:cursor-not-allowed disabled:opacity-50 pointer-coarse:h-11',
        active
