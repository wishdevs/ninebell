...
export const AGENTS: readonly Agent[] = [
  ...
  },
  {
    ...
    // steps는 flowGraph(CORPORATE_CARD_FLOW)와 동일한 노드 시퀀스 — 접힘(간략)과
    // 펼침(그래프)이 같은 플로우를 가리키도록 일치시킨다. 펼침 그래프가 기준.
    steps: [
      {
        id: 'access',
        label: '결의서 입력 접속',
      ...
      },
      {
        ...
        skill: '코드피커',
        status: 'done',
        detail: '간이·빈칸이면 02 재선택(증빙유형으로 회귀)',
      },
      {
        id: 'both',
        label: '매입·불공 동시?',
      ...
      },
      {
        ...
        skill: '필드 입력',
        status: 'done',
        detail: '증빙유형 01(매입) 행의 카드내역·금액 입력',
      },
      {
        id: 'rowAdd',
        label: 'F3 줄 추가',
      ...
      },
      {
        ...
        skill: '코드피커',
        status: 'done',
        detail: '추가된 행을 증빙유형 02(불공)로 별도 등록',
      },
      {
        id: 'budget',
        label: '예산계정 매핑',
      ...
      { id: 'warn', label: '저장 시 경고?', status: 'pending', detail: '회계일자·사용일 확인' },
      {
        id: 'submit',
        label: '전자결재 상신',
        status: 'pending',
        detail: '입력 적용까지만 — 저장·상신은 사람이',
      },
    ],
    logs: [
      {
