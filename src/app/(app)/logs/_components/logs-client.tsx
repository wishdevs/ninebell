...
/** `GET /logs` 한 행 — 접속 로그 응답 계약(camelCase)과 1:1. */
interface AccessLog {
  id: string;
...
/**
 * 접속 로그 테이블. logs:read(admin+) 권한이 없으면 접근 불가 상태를 보여주고
 * fetch 자체를 하지 않는다. 권한이 있으면 `GET /logs?limit&offset`로 최신순
 * 페이지를 불러오고 "더 보기"로 이어서 적재한다.
 */
    ...
    <div className="animate-page-enter flex flex-col gap-8">
      ...
      {!canRead ? (
        <EmptyState
          ...
          title="접근 권한이 없습니다"
          description="접속 로그는 관리자 이상만 열람할 수 있습니다."
        />
      ) : phase === 'loading' ? (
