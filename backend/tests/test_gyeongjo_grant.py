...
from app.agents.gyeongjo_grant import steps as gj_steps  # noqa: E402
from app.agents.gyeongjo_grant.nodes.gubun import make_confirmed_set_gubun_node  # noqa: E402
...
async def test_set_row_note_ok_when_cell_reflects():
    page = _GridPage()
    r = await gj_steps.set_row_note(page, "경조금-홍길동")
    assert r["ok"] is True and "warn" not in r
    assert page.cells["NOTE_DC"] == "경조금-홍길동"
...
async def test_fill_budget_ok_when_bg_cd_reflects(monkeypatch):
    ...
    r = await gj_steps.fill_budget_fixed(page, "회계팀", "판관비")
...
class _GubunPage:
    ...
    async def evaluate(self, js_src, arg=None):
        ...
        if js_src is js_lib.SELECTED_TEXT_JS:
            ...
            return {"ok": True, "text": self.selected, "value": "55"}
...
async def test_set_gubun_node_ok_when_selection_confirmed():
    events = asyncio.Queue()
    node = make_confirmed_set_gubun_node(GYEONGJO_GUBUN_LABEL)
...
async def test_fill_rows_surfaces_unverified_fields(monkeypatch):
    ...
    async def _note_unverified(page, text):
        calls.append("set_row_note")
        notes.append(text)
        return {"ok": True, "warn": "적요(NOTE_DC) 확인 불가(기대 '경조금-홍길동' / 실제 None…)"}
...
async def test_fill_rows_surfaces_warns_from_evdn_partner_project(monkeypatch):
    ...
    # '경조금 반영 완료'(ok) 로그와 fill_warnings 가 거짓이 되는 자체 불변식 위반.
    calls: list = []
    notes: list = []
    _patch_fill_all_ok(monkeypatch, calls, notes)
