...
AGENT_FIXTURES: list[dict] = [
    {
        ...
        "steps": [
            {"key": "login", "label": "로그인", "skill": "login", "status": "done", "detail": "더존 옴니솔 인증 후 세션 확보"},
            {"key": "user_type", "label": "회계 사용자 전환", "skill": "user-type", "status": "done", "detail": "사용자 유형을 '회계'로 전환"},
            {"key": "menu_nav", "label": "결의서입력 화면", "skill": "menu-nav", "status": "done", "detail": "전사공통(회계) 결의서 입력 화면 진입"},
            {"key": "set_gubun", "label": "결의구분: 카드", "skill": "field-input", "status": "done", "detail": "결의구분 드롭다운을 카드로 설정"},
            {"key": "add_row", "label": "상세행 추가(F3)", "skill": "field-input", "status": "done", "detail": "F3로 카드 결의 상세행 생성"},
            {"key": "set_acct_date", "label": "회계일 설정", "skill": "field-input", "status": "done", "detail": "수집 기간 월의 말일로 결의서 회계일 설정(전월 수집=전월 말일 / 당월=당월 말일)"},
            {"key": "open_evdn", "label": "증빙유형 선택", "skill": "codepicker", "status": "done", "detail": "증빙유형 코드피커 열기"},
            {"key": "select_evdn", "label": "법인카드 선택", "skill": "codepicker", "status": "done", "detail": "증빙유형 01 법인카드 선택 → 승인내역 팝업"},
            {"key": "select_all_cards", "label": "카드 전체선택", "skill": "codepicker", "status": "done", "detail": "카드번호 돋보기 → 전체선택 → 적용"},
            {"key": "set_period", "label": "승인일 기간", "skill": "field-input", "status": "done", "detail": "10일 미만=전월 / 10일부터=당월 기간 설정"},
            {"key": "query", "label": "조회", "skill": "grid-read", "status": "done", "detail": "승인내역 조회 → 리스트 보고"},
            {
                "key": "collect_rows", "label": "건별 입력(그리드)", "skill": "grid-input", "status": "active",
                "intervention": True,
                "detail": "전 행 예산단위·프로젝트·적요를 입력하고 '입력 완료'로 제출(사용자 개입).",
            },
            {"key": "apply_doc", "label": "과세분 적용", "skill": "doc-apply", "status": "pending", "detail": "과세 행 체크 → 적용 → 결의서 반영(저장 전)"},
            {"key": "switch_evdn", "label": "불공 전환", "skill": "codepicker", "status": "pending", "detail": "행 추가(F3) → 증빙유형 법인카드(불공) 선택 → 재조회·행 매칭"},
            {"key": "apply_pass2", "label": "불공분 반영·적용", "skill": "grid-input", "status": "pending", "detail": "입력해둔 값을 불공 행에 자동 반영 후 결의서 적용"},
            {"key": "save_final", "label": "저장(F7)", "skill": "save", "status": "pending", "detail": "과세·불공 반영분을 마지막에 한 번만 저장"},
        ],
        "logs": [
