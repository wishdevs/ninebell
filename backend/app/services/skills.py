...
SKILLS: dict[str, SkillDef] = {
    ...
    for s in (
        ...
        _skill(
            "eap-submit",
            "전자결재 상신",
            "전표의 결제창(전자결재)을 열어 상신 버튼을 클릭해 결재 프로세스를 시작하는 스킬. "
            "전표조회승인 계열 3종만 사용하며, 보관 버튼은 클릭하지 않는다.",
        ),
        _skill(
            "eap-cancel",
            "상신 회수",
            "전자결재 상신문서함에서 결재취소 → 상신취소 → 임시보관문서 삭제 3단계로 상신한 문서를 "
            "되돌리는 스킬(비가역). 디버그 전용 eap_cancel 에이전트만 사용한다.",
        ),
        _skill(
            "doc-delete",
            "문서 삭제",
            "결의서입력 화면에서 본인 작성·미결 문서를 조회해 삭제(F6)하는 스킬. "
            "테스트 문서 정리(디버그 전용) 에이전트만 사용한다.",
        ),
    )
}
