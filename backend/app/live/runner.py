...
# 셀렉터가 화면 밖으로 밀릴 수 있어 고정한다.
#
# 1440×900 → 1920×1200 확대(사용자 리포트 2026-08-14 "스크린샷이 깨져 보인다"). 종횡비는
# **16:10 그대로**라 프론트 스테이지(aspect-[16/10] + object-contain)는 손대지 않는다 —
# 레터박스 없이 그대로 채워진다. 넓힌 이유는 비율이 아니라 **폭 부족**이었다(실측 대조,
# 프로젝트BOM구매요청 화면):
#   1440 — 조회조건이 2줄로 접히고 그리드가 '구매요청수량'에서 잘림(그리드 폭 1162)
#   1920 — 조회조건 1줄, '요청금액'까지 노출(그리드 폭 1642)
# 좌표는 전부 요소 bbox 동적 계산이라(grid/strategies, js_lib) 확대해도 빗나가지 않는다.
# 스트리밍 대역은 불변 — CDP 스크린캐스트가 maxWidth/Height(1280×800)로 비율 유지 축소한다.
LIVE_VIEWPORT: dict[str, int] = {"width": 1920, "height": 1200}

# 자식 팝업(전자결재 결제창) 전용 뷰포트 — 결제 전표는 세로로 긴 문서라 부모 높이로는
# 아래가 잘린다. **폭은 부모와 같게**(아래 _cast_child 가 강제 리사이즈하는 이유) 두고 세로만
# 키워 한 프레임에 더 많은 폼이 렌더·캡처되게 한다(FE 는 고정 카드에 object-contain 으로
# 얹어 카드 너비와 무관하게 전체를 보여준다).
# 부모 확대(1440→1920)에 맞춰 함께 조정 — 종전 비율(부모 높이의 1.24배)을 유지한다.
CHILD_VIEWPORT: dict[str, int] = {"width": 1920, "height": 1500}
