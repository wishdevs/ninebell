...
이 화면의 조회 조건은 대부분 고정(회계단위=나인벨·전표상태=미결·전자결재상태=저장·전표유형=
국내/해외매출)이라 사용자 입력이 거의 없다. 파라미터는 두 가지다:
...
class VoucherReceivableParams(VoucherPeriodParams):
    """`params["voucher"]`(또는 flat params) — 처리 행 수 + 회계일 기간(상속).
...
def parse_voucher_params(params: dict | None) -> VoucherReceivableParams:
    ...
    max_rows=None(전체)·기간 미지정(화면 기본 당월). 실패는 한국어 ValueError(그래프 진입 노드가
    단락). (구 allow_batch 등 다른 키는 무시된다.)
    """
    raw = params or {}
    src: Any = raw.get("voucher") if isinstance(raw.get("voucher"), dict) else raw
    keys = ("max_rows", "period_from", "period_to")
