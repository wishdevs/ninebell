# 외상매출금 전표 결재 — 프로세스 정의 (voucher-receivable)

> 회계전표 그룹 에이전트. **결의서입력(GLDDOC00300)이 아니라** 총계정원장 > 전표관리 >
> **전표조회승인** 화면을 다룬다(신규 화면·신규 아키타입 — 문서 생성이 아니라 조회+결재).
> 상태: **v1 — 헤드리스 라이브 프로브 완료(2026-07-20), 배치(3건) 라이브 스모크 그린(2026-07-21).
> 진입·조회 8필드·결과그리드·결제창(별도 팝업 Page)·취소 비영속·배치순회(D7) ✅.
> 미해소: EAP draft 잔존·보관버튼 동작(❓).**
> 형제(같은 화면, 전표유형만 다름 — 향후): 외상매입금(`voucher-trade-payable`),
> 미지급금 법인카드(`voucher-card-payable`).

## 개요

전표조회승인 화면에서 **미결·전자결재저장 상태의 매출전표(국내/해외)**를 조회한 뒤,
목록을 한 건씩 돌며 **결제(결재) 창을 열어 전자결재로 실제 상신**한다(정책 변경
2026-08-07 — 종전 "가상 상신 로그만"에서 개방). 상신은 `loop_approvals` 의
**`allow_submit` 게이트** 뒤에 있고 기본값은 False(=가상 상신 로그만)이며, 회계전표
3종 그래프 빌더가 True 로 연다.

- 입력 방식: 실행 전 폼(조회 조건은 대부분 고정, 회계일만 월 선택). HITL 개입 없음(무개입 완주).
- 완주 조건: 조회된 전 건에 대해 (선택→결제창 열기→상신 클릭→자식창 닫힘 확인) 루프 완료.
- **FI 전표는 불변**: F7 저장·F6 삭제는 이 화면에 없다. 결제창에서 하는 일은 상신 하나이고,
  상신된 전자결재 문서는 EAP 에서 되돌릴 수 있다(아래 안전 섹션).

## ⚠ 비가역 / 안전

- **실제 상신(전자결재 상신) 실행 — 사용자 승인 2026-08-07.** 결제창(EAP) 상단 우측
  **상신**(파란 체크, ~922,30)을 실제로 클릭한다. 클릭 주체는 `steps.click_child_submit`:
  **fresh 좌표 재평가 → 클릭 → 확인 다이얼로그 best-effort 처리 → 자식창 닫힘 = 성공 판정**.
  실패하면 다음 행으로 넘어가지 않고 **런을 하드 중단**한다.
  - **가역 근거**: 상신문서 결재취소 → 미결문서 상신취소 → 임시보관문서 삭제까지
    `e2e/eap_approval_cancel_probe.py`(2026-08-07 신규, 2회 실검증 PASS)로 회수된다.
    비가역이 아니게 되었다는 것이 정책 개방의 유일한 근거다.
  - ❓ **상신 후속 다이얼로그·성공 신호는 미실측**이다. 확인 다이얼로그를 best-effort 로
    두고 **창닫힘**을 성공 판정으로 삼은 이유이며, 첫 라이브 실행에서 실제 문구·전이를 확정한다.
  - `allow_submit=False`(기본)이면 종전대로 가상 상신 로그만 남기고 닫는다.
- ⚠ **상신 후 행 소멸 → 재조회·인덱스 재매핑(사용자 리포트 2026-08-07)**: 실상신된 전표는
  조회 필터(**미결·전자결재저장**)를 벗어나 **결과 그리드에서 사라진다**. 초기 그리드 기준으로
  잡아 둔 인덱스가 그만큼 앞으로 당겨지므로, 가상 상신 시절의 "인덱스 고정" 가정이 깨진다
  (가상 상신은 행이 남아 문제가 없었다). 규율은 **D7 의 확장**으로 다룬다 — 아래 D7-3.
- **D7-3 상신 반영(allow_submit 전용)**: 그룹/건 **상신 성공마다 재조회(F2, `steps.run_query`)**
  로 그리드를 확정 상태로 만든다(마지막 그룹/건은 생략 — 뒤에 쓸 대상이 없다). 이후 대상
  인덱스는 **"이미 상신된 앞 행 수"만큼 차감**해 재매핑하고, **키를 아는 대상**(배치 그룹의
  `docu_nos`·카드의 선계획 키)은 **체크 직전 행 키(DOCU_NO) 대조**로 확정 검증한다.
  - 키 **불일치 = 하드 중단**("상신 반영 후 행 재확인 실패 — 예상/실제"). 키를 **못 읽으면**
    모호로 보고 대조를 생략한다(기존 D7 규율과 동일 — 리더 오탐으로 런을 끊지 않는다).
  - 재조회 후 **잔여 행수**는 기대값(직전 잔여 − 이번 상신 건수)과 대조하되 **soft(warn)**.
    잔여가 어긋나도 다음 대상은 키 대조로 확정하므로 여기서 끊지 않는다.
  - 게이트가 닫힌 경로(가상 상신)는 행이 남으므로 **재조회·재매핑을 하지 않는다**(종전 동작).
  - 구현: `nodes/approvals.py`·`nodes/approvals_batch.py`(`submitted_original` 누적·
    `remaining_expected`·`_row_key`). 회귀 테스트 4건 추가.
- **상신 개방은 이 계열(전표조회승인 3종) 한정이다.** 결의서입력 계열(card_collect·trip_* 등)
  다른 에이전트는 **상신 금지 그대로**다.
- **보관 버튼(~860,30)은 여전히 누르지 않는다** — 실제 저장/제출 여부 미확정(❓). 안전하게 미클릭 유지.
- 결제창 열기→닫기(`close()`)는 FI 전표에 **비영속 확정 ✅**(프로브 3회: DOCU_NO=FI2026070100000010
  이 미결/저장 불변). 이 화면엔 F7 저장·F6 삭제 자체가 없다.
- ⚠ **EAP draft 잔존 이슈(미해소)**: 결제창을 **여는 것만으로** 전자결재(EAP, `uc.ninebell.co.kr`)
  시스템에 새 임시문서(docID 1013899→1013905→1013906 매 오픈 증가)가 생기는 것으로 관찰됨. FI
  전표는 불변이나 EAP 임시/진행문서함에 draft 가 쌓일 수 있다. **배치 순회 전 도메인 담당자
  확인 필요** — 문제면 (a) 순회 안 함, (b) 열지 않고 목록만, (c) EAP draft 정리 로직 추가 중 택일.
  ❓ **상신을 실제로 실행하면 이 draft 가 상신문서로 넘어가 잔존이 줄어드는지는 미확정**이다
  (상신 개방 전 관측이라 재측정 필요). 회수 수단은 `e2e/eap_approval_cancel_probe.py` 가 있다.
- 테스트는 격리 계정(이트라이브2/1111)에서만.
- ⚠ **처리 범위: 전체 진행(사용자 결정 2026-07-21)** — max_rows 게이트/`allow_batch` 제거, 기본이
  조회된 **전 건 순회**다(params.max_rows=None). 즉 한 실행이 대상 건수만큼 결제창을 연다
  (예 31건이면 31회). **2026-08-07 부터는 그 전 건이 실제 상신된다** — 회수는 EAP 결재취소
  프로브로 한다. EAP draft 잔존 처리(위 이슈)는 담당자 확인 대기 상태로 남는다 — 노출·실행은
  사용자 지시로 열려 있다.

## 확정된 업무 결정(사용자, 2026-07-20)

- **D1 화면**: 총계정원장 → 전표관리 → **전표조회승인**. menu_id **`GLDDOC00700`** /
  deeplink **`/FI/GLDDOC00700`**(재무회계 모듈) — ✅ 2026-07-20 프로브. `nbkit/omnisol/menu_schemas.py`
  `VOUCHER_RECEIVABLE` 등록됨. 사용자유형 **회계**. 진입판정 `.dews-ui-grid` ≥2(기존 로직 재사용). ✅
- **D2 조회 조건(고정)** — ✅ 2026-07-20 프로브. 8필드 셀렉터·세팅방식:
  - 회계단위: codepicker `#s_pc_cd`. 기본값 **"(주)나인벨"** 이미 선택 → **변경 불필요**. ✅
  - 작성부서 **전체**: multicodepicker `#s_wdept_cd` 돋보기→팝업 RealGrid `checkAll()`→적용(46건). ✅
  - 회계일 **실행 전 폼 지정 기간**(기본 = 당월 1일~말일): periodpicker `#s_period`.
    - 기간 미지정·**당월 전체**: 앱 API **`dewsControl.setMonth()`**(이번달 1일~말일 자동).
      ⚠ `YYYYMMDD` 타이핑 아님(GLDDOC00300 규칙과 다름). ✅ 프로브 확정 경로.
    - 그 외(월 일부 기간 예 7/1~7/5·과거월): `#s_period_startinput`/`_endinput` 직접 세팅 후
      **readback 확인**(불일치는 하드 실패). ❓ 실화면 미검증 — 카드 `#PERIOD_DT_C` 규약을 따름.
  - 작성자 **비움**: multicodepicker `#s_wrt_emp_no`, 앱 API **`dewsControl.clear()`**로 기본선택 제거. ✅
  - 역분개여부 **전체**: native kendo `select#s_revjrnz_yn` 기본값(value="") = 전체 → **변경 불필요**. ✅
  - 전표상태 **미결**: native kendo `select#s_docu_st_cd`, 기존 `KENDO_SET_DROPDOWN_BY_TEXT_JS`
    (text='미결', value="2") 재사용. ✅
  - 전자결재상태 **저장**: multicodepicker `#s_gwaprvlst_cd`(MA/P01300) 돋보기→팝업 `SYSDEF_NM==='저장'`
    checkRow→적용(code=1). ✅
  - 전표유형 **국내매출+해외매출**: multicodepicker `#s_docu_cd`(MA/P00620, **optional-area**).
    ⚠ 팝업 RealGrid 컬럼은 **`SYSDEF_NM`/`SYSDEF_CD`**(전자결재상태 팝업과 동일 범용 코드테이블,
    n=62) — 2026-07-20 프로브의 `DOCU_NM`/`DOCU_CD` 기록은 오기(라이브 스모크로 정정). checkRow
    는 `SYSDEF_NM=='국내매출'/'해외매출'`. **optional-area 라 열기 직전 `ensure_field_visible`
    (결과검증형: 보이는 `.dews-condition-panel-expand-button`을 좌→우로 하나씩, 전표유형이 보일
    때까지 시도, 이미 보이면 미클릭)로 가시성 보장** — 고정 1회 expand 는 다른 필드 조작 중 패널
    재접힘 레이스로 불안정. ✅ 2026-07-21 라이브 스모크
  - (전표번호 `#s_docu_no`: 8필드 아님, 공란 유지)
- **D3 조회 실행**: 기존 `selectors.BTN_LOOKUP`("button.main-button.lookup", F2) 재사용. 결과
  마스터그리드(index 0) 28컬럼 — 핵심: **DOCU_NO**·DOCU_ST_NM·GWAPRVLST_NM·DOCU_NM·ACTG_NO·
  DOCU_AM_SUM·WRT_DEPT_NM. 성공 = rowcount ≥ 0(0이면 처리대상 없음 → 정상완료). 실측 31행. ✅
- **D4 행 선택**: **체크박스 필요** — `grid.checkRow(idx, true)`(`__UUID` 기반). `setCurrent`만으론
  결재 대상으로 인식 안 됨. ✅
- **D5 결제창(자식창)**: 결재 버튼 **`button.main-button.approval`**(innerText "결재", ~1317,72) →
  **별도 팝업 브라우저 창**(`window.open`, cross-origin `uc.ninebell.co.kr` EAP/전자결재, SSO 경유) =
  **진짜 새 Playwright Page**(모달/iframe 아님). ✅ 감지: 결재 클릭 **전에** `context.on("page")`
  리스너 등록 → 새 Page 캡처. 렌더완료: 고정대기 금지 → 상단 버튼(미리보기/보관/상신) 텍스트가 뜰
  때까지 1s 폴링(상한 25s, SSO 리다이렉트+SPA 마운트 1~12s 편차). ✅
- **D5-1 결재라인 교차 지정(신규 2026-08-21 — 사용자 지시)**: 로그인 계정이 **이트라이브/
  이트라이브2** 면 상신 전에 상대 계정을 결재라인에 추가한다(그 외 계정 no-op). 실측
  (approval_line_probe_1/2/4a/4b + cleanup, 이트라이브2 계정):
  - 모달 진입(사용자 headed 시연으로 확정 2026-08-21): **결재란 사용자 셀 `td.lineUserInfo`**
    (텍스트 있는 첫 셀) DOM 클릭이 1차 — Playwright 자동 대기가 배치(45건) 본문 렌더 지연을
    흡수한다(라이브 배치 실패의 원인이 이 지연이었음). 폴백 = 전표 헤더 '결재' 라벨(≈770,262
    세로 셀 w40×h110 — 형태 h≥60 판별, 출현까지 폴링). 둘 다 같은 child Page 안
    **"결재라인 지정" DOM 오버레이 모달**(새 창 아님)을 띄운다. ✅
  - 모달 우상단 인원표 = **canvas(RealGrid)** → DOM 셀렉터 불가, 픽셀 산식으로 체크박스
    클릭(헤더 27.5px·행 29.2px·체크박스 x+10). 인사/기획팀 행 순서 실측: 빈행/남희곤/황정하/
    이트라이브(3)/이트라이브2(4). ✅ 상단 툴바(DOM) 병렬/**결재**/합의/수신참조/시행에서
    '결재' 클릭 = 체크 인원을 결재선에 추가 → 최하단 '저장' 으로 모달 닫힘(확인 다이얼로그
    없음, 결제창 헤더에 배지 즉시 반영). ✅
  - **라이브 패리티(3연속 라이브 실패 원인 — 2026-08-21 확정)**: 러너가 자식 결제창을
    **1920×1500 강제 리사이즈**(runner.py CHILD_VIEWPORT)해 소형 창 실측 좌표가 라이브에서
    어긋난다. 규율: 절대 좌표 금지 — 툴바는 **모달 제목 리프 앵커 + (title.y, +150] 상대창**,
    캔버스는 **넓은(w>400) 것 중 최상단 = 인원표**, 라벨·캔버스·툴바·저장·검증 **전 단계
    폴링**(단발 조회 금지 — 모달 제목이 떠도 RealGrid 캔버스 초기화가 늦고, 배치 본문 렌더도
    수 초 늦는다). e2e `voucher_approval_line_probe.py` 가 러너와 같은 뷰포트·다건 배치를
    기본 재현한다. ✅
  - ERP 제한: 선택 전표의 상세내역 합이 200건 초과면 결제창 자체가 거부된다("전표 상세내역
    200 건 이하만 결재 상신 가능" 다이얼로그 실측 — 상세 6044건 전표 포함 시). 에이전트는
    count_details 묶음 계획(하위 200 기준)으로 원래 회피한다.
  - **비영속 확정 ✅**: 상신 없이 결제창을 닫으면 지정이 소멸한다(새 세션 재오픈 실측 —
    cleanup 베이스라인이 원상태). → 결제창을 열 때마다 **상신 직전에 매번** 지정한다.
    **디버그(가상 상신)에서도 지정까지는 수행**하고 상신만 생략한다(사용자 확정 2026-08-21
    — 비영속이라 흔적 없음, 라이브 화면 리허설 용도). allow_submit=False 빌더(구 가상 상신
    전용 경로)에서는 실행하지 않는다.
  - **검증 게이트 ✅**: 저장 후 전표 헤더(DOM)에 대상 이름 리프가 정확일치로 **새로** 생긴다
    ('이트라이브' 와 '이트라이브2' 는 별개 리프 — 부분일치 오염 없음). 리프 수 증가로 판정
    하고, 실패 시 **상신하지 않고 런 하드 중단**(오지정 상신 금지). 캔버스 행 인덱스가 인원
    변동으로 밀리는 경우도 이 게이트가 잡는다. ❓ 잔여: 행 순서의 정렬 규칙(입퇴사 시 재실측).
  - 구현: `nbkit/patterns/approval_line_flow.py`(범용 메커니즘) + `steps.ensure_cross_approval_line`
    (교차 규칙 APPROVAL_LINE_CROSS/행 인덱스) + 두 결재 노드(approvals·approvals_batch)의
    submit_on 분기에서 `click_child_submit` 직전 호출. **카드(voucher-card)도 공유 백본이라
    자동 반영.**
- **D6 상신(개정 2026-08-07 — 종전 "가상 상신")**: 결제창에서 **상신(~922,30)을 실제로 클릭**한다
  (`steps.click_child_submit` — fresh 좌표 → 클릭 → 확인 다이얼로그 best-effort → **자식창 닫힘 =
  성공**, 실패 시 런 하드 중단). `loop_approvals(allow_submit=...)` 게이트로 열리며 기본값 False
  는 종전 동작("가상 상신: 전표 {DOCU_NO}" 로그 + `child.close()`)이다. 보관(~860,30) = ⚠ 여전히
  미클릭. 상신 자체는 `e2e/eap_approval_cancel_probe.py` 로 취소 가능하고, 후속 다이얼로그·성공
  신호는 ❓ 미실측(첫 라이브에서 확정). EAP draft 잔존 이슈(위 안전섹션)는 그대로 인지.
  - 개정 이력: 2026-07-20 "가상 상신(상신 절대 금지)" → **2026-08-07 실제 상신(게이트)**.
- **D7 순서/재개**: 인덱스 0..n-1 진행. ✅ 2026-07-21 배치 3건 라이브 스모크로 확정.
  - **행/팝업 정합성 하드체크 구현**(`nodes/approvals.py:loop_approvals`): (1) 결제 열기 직전
    체크된 행이 정확히 1개인지(`steps.checked_row_indexes`), (2) 결제창에 표시된 전표번호가
    대상 행 DOCU_NO 와 확정적으로(매치 1개) 일치하는지(`steps.read_child_docu_no`, 정규식
    `\bFI\d{8,}\b` 리프텍스트 스캔) — 확정 불일치 시 배치 즉시 중단(모호 0/2개+ 는 경고만).
  - ⚠ **근본원인(2번째 이상 반복에서 결제창 미출현)**: 도메인전문가 확정 — "결제 팝업을
    닫으면 본창에서 별도 처리가 진행되고 로딩이 걸린다. 그 로딩이 끝나기 전에 다음 행을
    체크하고 결제를 다시 호출해서 안 되는 것." 읽기전용 진단(`e2e/voucher_receivable_parent_loading_diag.py`)
    으로 재현: `close_child()` 직후 `.dews-loading-container`(자식: `.dews-loading-img`/
    `.dews-loading-text`) 스피너가 뜬다(관측 ~0~0.6s). 별도 진단(`e2e/voucher_receivable_open_approval_diag.py`)
    으로 `check_row`(setCurrent)가 트리거하는 `.dews-loading-bg` 도 결재 버튼 클릭을 가로챈다는
    것도 확인(elementFromPoint 실측). **수정**: `steps.settle_parent_after_child_close`(close 직후
    호출)가 `child.is_closed()` 확인 → **로딩 인디케이터 소거 대기**(`wait_loading_overlay_gone`,
    `.dews-loading-bg`/`.dews-loading-container`/`.k-loading-mask`/`.k-loading`/`.dews-loading`
    전부 체크) → `bring_to_front()` → 결재버튼 rect 재가시 확인 순서로 정착. `steps.open_approval`
    도 매 시도(기본 2회, 결과검증형)마다 로딩 소거 대기 + rect **fresh 재평가**(캐시 금지) 후
    클릭. ✅ 3건 배치 3/3 성공(재발 없음).

## 신규 프론트 요구 — 라이브뷰 부모/자식 창 탭

라이브 브라우저 화면 **우측 상단**에 부모창/자식창을 나타내는 **탭(버튼)**을 두고 번갈아 볼 수
있게 한다. **결제창이 뜨면(결제 진행 중) 자식창 탭을 자동 활성화**해 결제창을 보여준다.
자식창이 닫히면 부모창으로 복귀(활성 탭 원복).

### 라이브 프레임 아키텍처 실측(탐색 2026-07-20)
현재 파이프라인은 **단일 페이지** 캡처다 — `app/live/runner.py`가 헤드리스 Chromium 페이지 1개를
열고, `app/live/screencast.py:screencast_pump(page, events)`가 그 페이지에 CDP 세션 1개를 바인딩.
프레임은 `{"screenshot": "data:image/jpeg;base64,..."}`(events.py)로 **창 식별자 없음**. 기존
"팝업"은 전부 같은 페이지의 Kendo `.k-window` DOM 오버레이(이미 스크린샷에 포함). **실제 새
브라우저 창(window.open→별도 Playwright Page)을 다루는 코드는 전무**(`context.on("page")` 0건).

→ 결제창이 **(B) 진짜 새 창**이면(사용자 표현 "새 시스템 윈도우" = 유력) 신규 플러밍 필요:
1. `runner.py`: `context.on("page")`로 새 페이지 감지 + 페이지별 `screencast_pump` 기동(창 role/id 태깅).
2. 프레임 프로토콜: `{"screenshot", "window": "parent"|"child", "windowId"}` 키 추가
   (`app/live/events.py`, `nbkit/patterns/__init__.py:emit_shot`, `src/lib/live/types.ts`).
3. `app/live/session.py`: `latest_shot` 단일 슬롯 → 창별 맵(부모/자식 덮어쓰기 방지).
4. 프론트 `use-live-run.ts` 리듀서: `screenshot` 단일값 → `{parent, child}` + `activeWindow`
   (자식 프레임 첫 등장 시 자동 전환).
5. 프론트 `live-browser-stage.tsx`: 브라우저 크롬바(우측 상단 StatusBadge 옆)에 탭 토글 추가.
→ 결제창이 **(A) `.k-window` 모달**이면 캡처 변경 불필요, `{"window": ...}` 신호만 추가하면 됨.
**A/B 판정은 프로버 결과로 확정**(❓).

## 진입/신규 스텝(초안 — 프로브 전)

| 스텝 | 내용 | 성공 | 실패 | 검증 |
|---|---|---|---|---|
| login | 옴니솔 인증(공유 프리미티브) | 세션 확보 | 인증 실패 팝업 → 중단 | ✅(기존) |
| user_type | 사용자유형 '회계' 전환(공유) | 회계 모듈 접근 | — | ✅(기존) |
| menu_nav | 총계정원장>전표관리>전표조회승인 진입 | 조회 폼·그리드 표출 | 메뉴 없음/권한 | ❓ menu_id |
| set_query | 조회 조건 세팅(D2) | 전 필드 반영 | 필드 부재 | ❓ 각 필드 셀렉터·값 |
| run_query | 조회 버튼 | 결과 그리드 로딩 | 로딩 실패 | ❓ 버튼 좌표 |
| loop_row | 행 선택 | 행 활성 | — | ❓ 선택 방식 |
| open_pay | 결제 버튼 → 자식창 | 자식창 표출 | 창 안뜸 | ❓ 버튼·창유형 |
| submit | 상신 클릭 + 확인 다이얼로그 + 창닫힘 판정 | 자식창 닫힘·부모 복귀 | 창 안닫힘 → 런 중단 | ❓ 후속 다이얼로그 문구 |

## 남은 작업

- [x] 결제창 유형 확정 = **별도 팝업 Page**(scenario B). ✅
- [x] menu_id/deeplink, 조회 8필드 셀렉터·세팅, 조회 버튼, 결과 그리드 컬럼·행선택(checkRow). ✅
- [x] 결제 버튼·결제창 상신/취소, 취소 FI전표 비영속 확인. ✅
- [x] 그래프 코드화(entry 재사용 + set_query + run_query + loop). ✅
- [x] 배치 3건 순차 스모크로 D7(행/팝업 정합성) 확정. ✅ 2026-07-21
- [ ] **사용자 확인 필요**: (1) EAP draft 잔존 허용/정리 여부 (2) '보관' 버튼 금지 여부.
- [ ] 프론트 부모/자식 창 탭(scenario B 멀티페이지 플러밍) — 별도 라이브뷰 아키텍처 참조.

## 검증 로그

⚠ 아래 스모크 기록은 **상신 개방(2026-08-07) 이전**의 것이라 전부 `allow_submit=False`
상태(가상 상신 로그)에서 얻은 결과다. 상신 실클릭 경로의 라이브 근거는 아직 없다.

- 2026-07-21 배치 라이브 스모크 `e2e/voucher_receivable_smoke.py`(delay_scale=0.4, max_rows=3
  기본값): **최종 그린** — processed=3, DOCU_NO 3건 모두 상이(FI2026070100000010/11/12), D7
  정합성 확인 3/3 ✅(모호·불일치 0), D7 체크행수 확인 3/3 ✅, 자식 스크린샷 401프레임(3세트)·
  닫힘 전이 3회, 최종 result 성공(에러 0). 과정에서 3회 실패 후 근본원인 2건 확정·수정
  (전표유형 팝업 필드 `DOCU_NM`→`SYSDEF_NM` 오기 정정, 반복 결제 오픈 시 로딩 미대기) — 상세는
  위 D2·D7 항목. `pytest -k voucher` 59 passed.
- 2026-07-20 프로브 `e2e/voucher_receivable_probe.py`(3회 그린, delay_scale=0.4): 진입(GLDDOC00700)·
  조회 8필드·결과그리드(31행)·행 checkRow·결재버튼·결제창(별도 팝업 Page, EAP cross-origin)·창닫기
  FI전표 비영속(3회 불변) 확정. 미해소: EAP draft 매 오픈 증가(1013899→1013905→1013906), '보관'
  동작 미확인, 배치 순회 미검증. 아티팩트 `e2e/artifacts/voucher_receivable_probe_*.{png,json}`.
- 2026-07-21 관리자 단건 라이브 스모크 `e2e/voucher_receivable_smoke.py`(run_workflow 종단, 4회:
  3실패→분류→수정→PASS): **EAP 결제창이 라이브 러너 부모/자식 캡처로 잡힘**(context.on("page")→
  자식 프레임 172개), 가상 상신 로그 "가상 상신: 전표 FI2026070100000010", 자식 닫힘 전이 1회,
  최종 성공(에러 0), EAP draft 1건(max_rows=1 게이트 유지), 상신/보관 미클릭. 근본원인 2건 수정:
  (1) 전표유형 팝업 필드명 `DOCU_NM`→`SYSDEF_NM`, (2) 패널 재접힘 레이스 → `ensure_field_visible`.
  아티팩트 `e2e/artifacts/voucher_receivable_smoke_{parent,child}.png`·`_frames.json`.

## 배치 결재(외상매출금 전용, 사용자 요구 2026-07-27)

기존 "한 건씩 결제 올리기"를 **하위(계정정보) 건수 기반 묶음 결재**로 바꿨다.

### 규칙(사용자 확정)
1. 조회로 **전체 대상 건수**를 파악한다.
2. 각 전표의 **하위 그리드 건수**를 파악한다.
3. 하위 건수가 **단독 200 이상**인 전표는 **먼저 단독 결재**.
4. 나머지는 **합계가 200이 안 되도록**(< 200) 묶어 **일괄 결재**.

### 실측 근거(2026-07-27, 읽기전용 프로브 2종)
| 확인 | 결과 |
|---|---|
| 마스터 그리드에 하위 건수 컬럼 | **없음**(27컬럼 전수 확인) → 행별로 읽어야 함 |
| 하위 로딩 트리거 | `setCurrent` 만으로 재조회됨(checkRow 불필요) — 138행 전부 **개별 갱신 확인**(디테일 지문 138개 상이) |
| 행당 비용 | 평균 **372ms**(min 160·max 391) → 138건 전수 **52초** |
| 실제 분포 | 하위 **3~17건**(137건이 3~9, 1건만 10~49) · 합계 436 · **200 이상 0건** |
| 다중 선택 결재 | 2행 체크 → 결재 1회 → **자식창 1개**에 **두 전표번호 모두** 표시 = 묶음 성립 |
| 배치 라이브 | 5건 → **결재 1회**, D7 2종 모두 ✅(스모크 ALL PASS) |

→ 현재 데이터 기준 138건이 **3묶음**으로 처리된다(결재 138회 → 3회).
아티팩트: `e2e/artifacts/voucher_receivable_detail_counts.json`,
`e2e/artifacts/voucher_receivable_batch_approval.json`.

### 구현
- `batching.py` — `plan_approval_groups()`(순수): 단독(200↑·건수미상) **먼저**, 그 뒤 조회 순서
  그대로 순차 그리디로 합계 < 200 묶음. **정렬 재배치 없음**(로그·화면 대조 용이).
- `steps.read_detail_count()` — setCurrent 후, 디테일 첫 행의 `DOCU_NO` 가 **그 전표와 같아질
  때까지** 확인 커널로 대기한 뒤 건수를 채택한다. ⚠ 대부분 전표가 3라인이라 **건수만으로는
  스테일(이전 행 디테일)을 구분할 수 없다** — 소유 확인이 유일한 근거다. 확인 실패 시 건수
  미상(-1)으로 두고 그 행은 **단독 그룹**으로 뺀다(모르면 안전한 쪽).
- `nodes/count_details.py` — run_query 뒤에 삽입, `approval_plan` 산출(진행률 노출).
- `nodes/approvals_batch.py` — 그룹마다 전체 해제 → 대상 행 일괄 체크 → 결재 1회.

### D7(정합성) 확장
| 검사 | 배치 모드 |
|---|---|
| 체크 행 | 계획 그룹과 **정확히 일치**해야 함(불일치 = 하드 중단) |
| 결제창 전표 | **계획 밖 전표가 보이면 하드 중단**(다른 문서 혼입) |
| 일부 미표시 | 경고만(대량 배치 렌더/스크롤 편차 — 모호는 하드 근거로 쓰지 않는 기존 규율) |
| 상신 반영(D7-3) | 그룹 상신마다 재조회(F2) → 다음 그룹 인덱스 재매핑 → **체크 직전 그룹 `docu_nos` 키 대조**(불일치 = 하드 중단, 읽기 실패는 생략). 잔여 행수 대조는 soft |

### 적용 범위
**외상매출금만.** 외상매입금은 건별 순회 유지(그래프에 `count_details` 노드 없음), 미지급금
법인카드는 결제창마다 **행별 참조문서**를 다뤄 묶음과 양립하지 않으므로 제외
(`build_voucher_graph(batch_limit=...)` 로 매출금에서만 켠다).

⚠ 묶음 결재도 **상신은 실행한다**(2026-08-07 개방 — `allow_submit`). 묶음 하나가 결제창 1회이므로
상신도 묶음당 1회다. **보관은 여전히 미클릭.** 결제창 1회 = EAP 임시문서 1건 성격이므로,
건별 대비 **임시문서가 오히려 줄어든다**.
