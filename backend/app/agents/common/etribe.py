...
만 파싱한다. 인증은 Bearer 임의 토큰이되 **토큰이 사용자 식별자 = QoS 단위**(ETRIBE-LLM
API 문서: 토큰당 동시 4 요청) — 디스패처가 settings.etribe_token 을 넘긴다.
...
단 429(concurrency_limit)만 문서가 명시한 Retry-After(기본 5초)를 존중해 대기한다.
...
# Bearer 토큰 기본값 — 토큰=사용자 식별자(QoS 단위). 디스패처가 settings.etribe_token 을
# 넘기지 않는 직접 호출(테스트 등)용 폴백.
_DEFAULT_TOKEN = "ninebell-dashboard"
...
# 429 concurrency_limit 의 Retry-After 폴백(초) — 문서 명시값.
_RETRY_AFTER_DEFAULT_S = 5.0
...
async def _post_chat(http: Any, base: str, body: dict, *, tag: str, token: str) -> dict:
    ...
    429(concurrency_limit — 토큰당 동시 4 초과)는 지수 backoff 대신 서버 Retry-After
    (기본 5초)를 기다린다.
    """
    ...
    for attempt in range(1, _MAX_ATTEMPTS + 1):
        try:
            r = await http.post(url, headers=_headers(token), json=body)
            r.raise_for_status()
...
async def _decide_via_json_mode(
    ...
    body = {
        ...
        # API 권장 1.0 — 0 근처 저온은 thinking ON 에서 CoT 붕괴 위험(0만 서버가 0.9 보정,
        # 0.1 등은 보정 없이 그대로 전달됨). 도구 선택의 일관성은 스키마·프롬프트가 담당.
        "temperature": 1.0,
        "max_tokens": max_tokens,  # 사고 무제한 생성 방지(네이티브 경로와 동일).
        "response_format": {"type": "json_object"},
    }
    data = await _post_chat(http, base, body, tag="decide-json", token=token)
    ...
    *,
    ...
    if oa_tools and base_key in _JSON_FALLBACK_BASES:
        ...
        return await _decide_via_json_mode(
            http, model, base, system, content, tools, max_tokens=max_tokens, token=token
        )
    ...
    body: dict = {
        ...
        # API 권장 1.0(_decide_via_json_mode 와 동일 근거 — 저온은 CoT 붕괴 위험).
        "temperature": 1.0,
        "max_tokens": max_tokens,  # 사고 무제한 생성 방지(타임아웃 회피). 서버 상한 16384 클램프.
        # thinking 서버 기본 ON 유지 + reasoning_effort 미전송=자동(문서 권장 — 실작업은
        # 서버가 high 처리). 사고과정(reasoning_content)은 파싱에서 무시하고 tool_calls 만 읽는다.
    }
    ...
    try:
        data = await _post_chat(http, base, body, tag="decide", token=token)
    except httpx.HTTPStatusError as exc:
...
async def etribe_generate_text(
    ...
    *,
    system: str,
    user: str,
    # None = API 권장 기본 1.0(저온은 thinking ON 에서 CoT 붕괴 위험 — 모듈 헤더 참조).
    ...
    data = await _post_chat(http, base, body, tag="text", token=token)
    msg = ((data.get("choices") or [{}])[0].get("message")) or {}
