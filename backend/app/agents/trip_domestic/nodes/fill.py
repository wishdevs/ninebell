...
def make_fill_rows_node():
    """plan_rows 를 행별로 채운다 — 증빙(10)→거래처→예산단위→프로젝트→금액→적요→상대계정.

    통행료 거래처 = 카탈로그 코드/이름, 유류비 거래처·상대계정 = 작성자 본인(state['userid']).
    예산단위 = 부서 × 비용구분(판/제) 여비교통비-국내출장 고정 조합. 실패는 행·필드 명시 error.
    """
    ...
    async def fill_rows(state: dict) -> dict:
        ...
        for i, row in enumerate(plan_rows):
            ...
            kind_label = "통행료" if rtype == "toll" else "유류비 지원"
            await emit_log(events, f"{i + 1}/{total}행 입력 시작 — {kind_label} {amount:,}원.", "info")
