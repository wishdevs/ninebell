...
- **D3 회계일자(마스터 ACTG_DT)**: **계산서일(증빙일) 중 가장 마지막일**로 파생(사용자 입력 없음,
  개정 2026-07-07). compact 'YYYYMMDD' 로만 세팅(대시 형식은 셀을 비우는 함정 — card 실측).
- **D3b (세금)계산서일(detail `START_DT`)**: **행별 증빙일**(통행료/유류비 결제일, 사용자 입력).
  실행 전 폼 표에서 행마다 입력받아 fill_rows 가 detail 셀 `START_DT` 에 compact 로 setValue(ACTG_DT 계열).
...
- **D7 유류비 기준(에이전트 관리 설정)**: 차량별 기준연비 4종
  ...
  설정: 차량종류 동적 목록(fuel_classes = [{id,label,kmPerL}], 관리자 추가/삭제) + 기준단가(fuel_unit_price). 차량종류 4종은 기본값이며 관리 화면에서 추가·삭제 가능.
...
9. **fill_rows** — `plan_rows` 내부 루프(HITL 없음):
   ...
   - 행별 채움 순서: 증빙유형(10) → **계산서일(START_DT)** → 거래처 → 예산단위(고정) → 프로젝트
     → 거래금액 → 적요 → 상대계정거래처(본인).
...
| 회계일(마스터) | `SET_ACCT_DATE_JS` — `ds.setValue(0,'ACTG_DT','YYYYMMDD')` compact 전용. 값=계산서일 최댓값(파생) | ✅ P3 |
| (세금)계산서일 | detail 셀 **`START_DT`**(헤더 "(세금)계산서일", 프로브 trip_amount_cols) — `set_invoice_date` compact setValue+검증 | ✅ v9 |
...
- v9 (2026-07-07): **(세금)계산서일(증빙일) 행별 입력 추가 + 회계일자 파생 전환**(사용자 요청).
  - **D3 개정**: 회계일자는 입력받지 않고 **계산서일 최댓값**으로 파생(`parse_trip_params`).
  - **D3b 신규**: detail 셀 `START_DT`(헤더 "(세금)계산서일", 프로브 trip_amount_cols 실측)에 행별
    증빙일(통행료/유류비 결제일)을 compact setValue+검증(`steps.set_invoice_date`). `fill_rows` step 2.5 배선.
  - **params**: `TollRowIn/FuelRowIn.invoiceDate: date` 추가, `TripParams.acctDate` 제거, plan_row 에
    `invoiceDate`(compact) 포함. 계산서일 누락은 한국어 ValueError.
  - **프론트**: 실행 전 폼을 **표(그리드) 1행/줄** 레이아웃으로 개편(회계일자 필드 제거, 행별 계산서일
    date 셀 추가, 회계일자=마지막 계산서일 미리보기). 미실행(pre-run) 상태에서 라이브 브라우저 열 축소·
    입력 표 확대(개입 레이아웃 재사용, `agent-detail-client` panelWide). 단위 테스트 갱신(params/nodes/
