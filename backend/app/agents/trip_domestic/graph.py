"""출장(국내/자차) 결의서입력(trip-domestic) — LangGraph StateGraph 조립.

card_collect 와 달리 HITL 이 없다(모든 입력은 실행 전 폼 → params). 진입 앞단(login→user_type
→menu_nav→set_gubun→add_row)은 app.agents.common.nodes 를 재사용하고, 회계일·행 채움·저장만
신규다. 증빙(open/select)은 fill_rows 내부에서 행별로 처리한다(P9: carry-over 없음).

체인: validate_params(브라우저 앞) → login → user_type(회계) → menu_nav → set_gubun(출장(국내·자차))
...
# 결의구분 라벨(P1 실측): 가운뎃점 `·` (슬래시 아님), value 53.
TRIP_GUBUN_LABEL = "출장(국내·자차)"

# 재시도 루프 상한 — 체인 ~10노드 × (초기+2재시도) ≈ 30 < 40.
RECURSION_LIMIT = 40


class TripDomesticState(BaseAgentState, total=False):
    """러너 주입 공통 키(page/…/result/error)는 BaseAgentState 상속.

    ⚠ 노드가 반환하는 키는 전부 여기 선언돼야 다음 노드로 전달된다(LangGraph 미선언 키 silent drop).
    """

    plan_rows: list[dict]  # validate_params 산출 — 정규화된 행 목록(type/partner/amount/project/note/km/carClass)
...
def build_trip_domestic_graph():
    """출장(국내/자차) 체인을 컴파일해 반환(stateless·재사용 가능)."""
    g = StateGraph(TripDomesticState)
    ...
    def _after_save(state: TripDomesticState) -> str:
