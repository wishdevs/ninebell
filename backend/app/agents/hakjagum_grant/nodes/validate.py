...
실행 전 폼이 보낸 `state["params"]["hakjagum"]` 를 pydantic 으로 검증·정규화해 plan_rows(단건)로
만든다(공급가액 = 사용자 입력 금액 그대로 — 50% 규칙 없음, 경조금과 차이). 예산단위 조합매칭에
필요한 부서(department)·비용구분(cost_type)이 있는지 확인한다. 실패는 한국어 error 로 단락(이후
노드가 전부 건너뛰고 러너가 error 프레임으로 종료).
"""
...
from ..params import parse_hakjagum_params
...
def make_validate_params_node():
    ...
    async def validate_params(state: dict) -> dict:
        ...
        try:
            plan_rows, acct_compact = parse_hakjagum_params(params)
        ...
        if not department:
            await emit_step(events, "validate_params", "failed")
            return {
                "error": "사용자 부서 정보가 없습니다 — 예산단위(복리후생비-기타) 조합을 "
