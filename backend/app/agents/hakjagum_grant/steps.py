...
(확인 커널 적용분도 그 단일소스에서 승계된다 — 여기서 다시 확인하지 않는다). 학자금 고유 델타는
**예산계정 base('복리후생비-기타')·검색어('복리후생비')** 뿐이라 예산단위 스텝
(bgacct_name_for_cost_type·fill_budget_fixed)만 재정의한다. 매칭 함수(pick_budget_row)와 피커 공용 헬퍼
(_open_detail_cell_picker·_fail_close·_search_and_pick·_select_and_apply)도 국내출장 것을 재사용한다.
"""
...
# 이미 들어 있다 — 여기서는 **학자금 델타(예산단위)에만** 추가 확인을 붙인다.
# ══════════════════════════════════════════════════════════════════════════════
