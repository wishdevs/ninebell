"""학자금신청서 결의서입력(hakjagum-grant) — LangGraph StateGraph 조립.

경조금(gyeongjo_grant)과 기본틀 동일 — HITL 없음(모든 입력은 실행 전 폼 → params). 진입 앞단
(login→user_type→menu_nav→set_gubun→add_row)은 app.agents.common.nodes 를 재사용하고, 회계일·
단건 채움·저장만 신규다. 증빙(open/select)은 fill_rows 내부에서 처리한다.

학자금 델타: 결의구분 라벨(학자금신청서)·단건(1행)·거래처 본인·예산단위 복리후생비-기타·
공급가액 사용자 입력 그대로(50% 규칙 없음)·적요 '학자금-{본인}'. 회계일자 = 증빙일자(사용자 입력).

체인: validate_params(브라우저 앞) → login → user_type(회계) → menu_nav → set_gubun(학자금신청서)
...
# 결의구분 라벨 — 형제 카탈로그(결의구분 목록) 라벨 텍스트 기준 "학자금신청서"(라벨 완전일치 세팅).
# value 코드는 코드에서 미사용 — 라이브 프로브 실측(2026-07-15, hakjagum_probe_results.json D3):
# 라벨 "학자금신청서" 정확 존재, value="56"(경조금신청서 55 바로 다음), set_result ok. 검증:✅.
HAKJAGUM_GUBUN_LABEL = "학자금신청서"

# 재시도 루프 상한 — 체인 ~9노드 × (초기+2재시도) ≈ 27 < 40.
RECURSION_LIMIT = 40


class HakjagumGrantState(BaseAgentState, total=False):
    """러너 주입 공통 키(page/…/result/error)는 BaseAgentState 상속.

    ⚠ 노드가 반환하는 키는 전부 여기 선언돼야 다음 노드로 전달된다(LangGraph 미선언 키 silent drop).
    """

    plan_rows: list[dict]  # validate_params 산출 — 정규화된 단건 행(invoiceDate/amount/project)
...
def build_hakjagum_grant_graph():
    """학자금신청서 체인을 컴파일해 반환(stateless·재사용 가능)."""
    g = StateGraph(HakjagumGrantState)
    ...
    g.add_node("set_gubun", make_confirmed_set_gubun_node(HAKJAGUM_GUBUN_LABEL))
    ...
    def _after_save(state: HakjagumGrantState) -> str:
