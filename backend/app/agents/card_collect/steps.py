...
def owner_name_variants(
    ...
    패널 이름이 "석대현 프로"처럼 직책을 달고 있어(사용자 확정 2026-07-29) 순수 이름
    변형이 필요하다. ⚠ "이름 직책" 변형은 원래 소유자 컬럼 "석대현 프로" 표기 대응이었는데,
    2026-08-13 매칭을 카드명 FINPRODUCT_NM 괄호로 제한하면서(오탐 반증 — js 참조) 카드명
    괄호에 직책이 붙는 표기가 없는 한 죽은 값이다. 무해해서 코드는 유지(최소 diff).
    중복 제거·빈 값 제외, 순서 보존.
    """
...
async def select_all_cards(
    ...
    by='name'|'all'. 매칭은 카드명 FINPRODUCT_NM("…법인카드(석대현)-2826") 괄호 '(이름)'
    포함 **하나뿐** — 종전의 CARD_OWNR_NM/KOR_NM/PARTNER_NM 컬럼 매칭은 2026-08-13 실측으로
    반증·제거됐다(공용카드 행이 KOR_NM='정원호' 를 채워 와 남의 카드 오탐,
