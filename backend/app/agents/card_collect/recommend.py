...
# 신뢰도 게이트 **임시 비활성**(2026-08-05 사용자 지시) — 저신뢰 추천도 그대로 적용한다.
# 애매 가맹점(KCP 결제대행·구독 등)의 0.3~0.5 추측도 빈 칸보다 낫다는 판단. 다시 켜려면
# True 로만 바꾸면 된다(임계값·게이트 로직은 그대로 유지).
RECOMMEND_CONFIDENCE_GATE = False
...
_SYSTEM = (
    ...
    # ⚠ 사규 변경 2026-08-13: **중식 폐지** — 점심 결제도 석식(연장근무 식대) 계정으로 처리한다.
    #   (시스템도 meal_time.slot_for_time 이 11시 이후를 전부 석식으로 확정한다.)
    "행의 time 은 거래시각(HH:MM:SS)입니다. 후보 계정이 식사 시간대로 나뉘어 있으면 "
    "**반드시 이 시각으로 구분**하세요 — 11시 이전은 조식, **11시 이후는 모두 석식**입니다. "
    "사규 변경으로 '중식' 계정은 더 이상 쓰지 않습니다: 점심시간(11~15시) 결제도 석식 계정을 "
    "고르세요. 가맹점명이나 과거 선택만 보고 시간대를 단정하지 마세요. "
    ...
    "다만 priorChoice 가 **시간대로 나뉘는 계정**(조식/중식/석식 등)이고 이 거래의 time 이 그 "
    "시간대와 맞지 않으면, 과거 선택을 그대로 쓰지 말고 **같은 성격의 계정 중 시각에 맞는 것**을 "
    "고르세요. priorChoice 가 '복리후생비-중식'이면 시각과 무관하게 '복리후생비-석식'을 고르세요 "
    "(중식 폐지 — 과거 확정 이력에는 아직 중식이 남아 있습니다). "
