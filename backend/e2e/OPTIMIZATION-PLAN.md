...
1. **apply_doc 82s 규명·축소** — 🔧진행중. `apply_rows_to_document`에 계측(clicks/iters/elapsed_ms)
   ...
   (b)면 불가피(문서화).**
2. **AI 추천 28.5s 스킵** — ✅완료(범위 축소). **원칙(사용자): 기본지정/비용구분은 '기본'일 뿐
   최종값이 될 수 없다.** 따라서 기본만 있는 행은 반드시 AI 가 가맹점을 평가한다. AI 를 건너뛰는
   경우는 오직 **전 행이 학습 Tier-1(반복 확정 count>=MIN)로 결정적 커버**될 때뿐(그땐 AI 결과가
   어차피 안 쓰임 — 학습 우선). `_prefill_selections`에서 recommend 호출 전에 `all_covered` 판정.
   회귀 테스트 2건(학습만 스킵 / 기본만이면 AI 호출) 추가.
   ⚠ **절감 실현 조건**: 학습 데이터가 쌓여야 발동 → 신규/콜드 런은 여전히 AI 28.5s 발생.
   블랭킷 28.5s 절감이 아니라 **반복 사용 시 점증**하는 최적화로 정정.
