"""HEADLESS 읽기전용(부작용 0) 프로브 — 학자금신청서 결의서입력 필드 토폴로지 실측.
...
한다(중복정의 금지). 경조금 프로브(gyeongjo_probe.py)의 학자금 델타판 — 신규 작성 JS 는 경조금과
동일한 3개뿐(GRID_COLUMNS_JS editor 확장·AMOUNT_EDITOR_DUMP_JS 전량 덤프판·COUNTER_LABEL_EXISTS_JS
부작용 없는 존재확인판) — 모듈 하단 "재사용 소스 지도" 주석 참조. 프로덕션 조건 재현:
LIVE_VIEWPORT(1440x900) + _ScaledPage(delay_scale=0.4)(trip_agent_validate.py 와 동일 패턴,
app.live.runner 재사용).

확인 대상(hakjagum_grant ❓):
  1. D3 결의구분 "학자금신청서" 정확 라벨·value(옵션 전량 덤프로 실측)
  2. detail 그리드 getColumns() 전량(경조금 대비 동일 컬럼셋인지)
  3. D8 예산단위 피커에서 "복리후생비"/"기타" 검색 → (판)/(제)복리후생비-기타 의 정확
     BGACCT_NM + BGACCT_CD(BGACCT_NM 부분포함 매칭, 검색어별 결과행수 포함)
  4. D5 증빙유형 코드 10 라벨 재확인
...
Usage:
    cd /Users/wishdev/et-works/dashboard-design/.claude/worktrees/precious-hopping-treehouse/backend
    /Users/wishdev/et-works/dashboard-design/backend/.venv/bin/python e2e/hakjagum_probe.py
"""
...
GUBUN_LABEL = "학자금신청서"
EVDN_CODE = "10"
# 학자금 예산단위는 정확 코드 미확정(경조금 2005 같은 확정 코드 없음) — BGACCT_NM 부분포함으로
# "(판)/(제)복리후생비-기타"를 전부 수집해 프로브 결과에서 정확 BGACCT_CD/BGACCT_NM 을 확정한다.
BUDGET_NM_BASE = "복리후생비-기타"
BUDGET_SEARCH_KEYWORDS = ["복리후생비", "기타"]

# ── 신규 작성분(경조금 프로브와 동일 3개) — 나머지 발견용 JS 는 상단에서 e2e.trip_probe 를 그대로
...
async def _dump(name: str, data) -> None:
    path = ARTIFACTS / f"hakjagum_probe_{name}.json"
...
async def _shot(page: Page, name: str) -> None:
    try:
        p = str(ARTIFACTS / f"hakjagum_probe_{name}.png")
...
# trip 고유 로직이 없어 gyeongjo/hakjagum 에도 그대로 통한다(경조금 실측 확인 선례).
...
async def main() -> None:
    ...
    page = _ScaledPage(raw_page, DELAY_SCALE)  # 프로덕션 조건 재현(hakjagum-grant 예정 delay_scale=0.4)
    ...
    try:
        ...
        if not chosen:
            ...
            print("[FATAL] 결의구분 옵션에 '학자금신청서' 없음 — 즉시 중단(원인분류: 필드부재/문서종류 편차)", flush=True)
            return
        ...
        for attempt in range(1, 4):
            ...
            if d5["opened"]:
                break
        if d5["opened"]:
            # 조건 폴링(SKILL.md §4) — 팝업이 뜬 직후엔 "데이터 로딩 중" 스피너 상태로 rowcount=0
            # 일 수 있다(2026-07-15 attempt 1 실측: 고정 600ms 로는 미스). n>0 이 될 때까지 폴링.
        ...
        # ── 4) D8: 예산단위 피커 — "복리후생비"/"기타" 검색 → BGACCT_NM 부분포함 매칭 ─
        print("\n===== D8: 예산단위 복리후생비/기타 =====", flush=True)
        d8: dict = {}
        ...
        if op.get("ok"):
            ...
            for kw in BUDGET_SEARCH_KEYWORDS:
                ...
                # 정확 코드 미확정 → BGACCT_NM 에 "복리후생비-기타" 부분포함되는 행을 전부 수집
                # ((판)/(제) 접두 변형 포함) — BGACCT_CD/BGACCT_NM 전체를 실측 기록.
                matches = [
                    {"BGACCT_CD": o.get("BGACCT_CD"), "BGACCT_NM": o.get("BGACCT_NM"),
                     "BG_CD": o.get("BG_CD"), "BG_NM": o.get("BG_NM")}
                    for o in options
                    if BUDGET_NM_BASE in str(o.get("BGACCT_NM") or "")
                ]
                d8[f"search_{kw}"] = {
                    "rows": read.get("rows"),
                    "n_options": len(options),
                    "matches_nm_contains": matches,
                    "sample": options[:10],
                }
                print(f"[D8] '{kw}' -> rows={read.get('rows')} n_opts={len(options)} matches({BUDGET_NM_BASE})={matches}", flush=True)
