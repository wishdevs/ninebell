"""경조금신청서 실저장 사이클 스모크 — 그래프 완주(F7 실저장)→검증→삭제 N회 반복.

trip_smoke_cycle.py(국내출장) 포팅. 정책은 동일: 저장 없는 반복은 무의미 → **실저장(F7)→검증→
삭제** 사이클로 테스트한다. F7 실저장이 테스트에서 명시 승인됨(단 반드시 F6 삭제로 정리, 상신 금지).

경조금 델타(trip 대비): 결의구분 55(출장 53) · 단건 params(gyeongjo 네임스페이스) · 저장 후
detail SPPRC_AMT2(공급가액)가 근속<1년 50%(Decimal ROUND_HALF_UP) 계산값과 실제로 일치하는지 사이클
마다 검증. **상대계정거래처는 경조금엔 불필요(2026-07-13 사용자 확정)** — fill 노드가
register_counter_partner·delete_blank_row 를 더 이상 호출하지 않으므로 BFC_PARTNER_CD 는 검사하지
않는다. 대신 그 스텝이 만들던 **스트레이 빈 행이 이제 없어야 한다** — 사이클마다 detail 행수(=1)로
검증한다.

⚠ 안전 수칙
  - **삭제까지가 한 사이클** — 삭제 검증(잔존 0) 없이는 다음 사이클 진행 금지.
  - **상신(결재) 절대 금지** — 삭제 불가 상태를 만들지 않는다. F7(저장)·F6(삭제)만.
  - 삭제 가드레일: 결의자(WRT_EMP_NM)=로그인계정 + 결의구분(ABDOCU_FG_CD)=55(경조금신청서) +
...
Usage: cd backend && GYEONGJO_SMOKE_CYCLES=10 .venv/bin/python e2e/gyeongjo_smoke_cycle.py
  (GYEONGJO_SMOKE_CYCLES 미지정 시 기본 1 — 단발 검증용)
"""
...
from app.agents import build_gyeongjo_grant_graph  # noqa: E402
...
GYEONGJO_FG = "55"  # 결의구분 경조금신청서 코드(단계 2 프로브 실측, gyeongjo_probe.py).
...
# 스텝 제거(사용자 확정)로 BFC_PARTNER_CD 는 더 이상 의미 있는 신호가 아니라 덤프에서 뺐다. `n`
...
# 경계값**을 명시로 섞는다(100001→50001·300001→150001·250001→125001 은 사용자가 지정한 예시,
# 71001·133333 은 동일 패턴의 추가 경계값). under1Year=False 사이클은 정액이 그대로 저장되는지
# (반올림 미적용) 검증한다. cycle 이 표보다 많으면 순환 재사용.
...
def _cycle_params(cycle: int) -> dict:
    ...
    expected = supply_amount(base_amount, under1)
    return {
        "gyeongjo": {
            "evidenceDate": TODAY,
        ...
        "_summary": {"baseAmount": base_amount, "under1Year": under1, "expected_supply": expected},
    }


def _row_is_ours(row: dict) -> bool:
    """삭제 안전 가드 — 결의자=USERID + 결의구분=55(경조금신청서) + 미결(DOCU_NO 공백)."""
    writer_ok = str(row.get("WRT_EMP_NM") or "").strip() == USERID
    fg_ok = str(row.get("ABDOCU_FG_CD") or "") == GYEONGJO_FG
...
async def _drive_graph(page, params: dict) -> dict:
    ...
    graph = build_gyeongjo_grant_graph()
...
async def _verify_and_delete(page, cycle: int) -> dict:
    """저장된 경조금 결의를 조회→가드레일 검증→F6 삭제→잔존 0 확인. 반환 진단 dict."""
    ...
    if not all_ours:
        ...
        out["dump"] = dump
        await page.screenshot(path=str(ART / f"gyeongjo_save_c{cycle}_guardrail.png"))
        return out
    ...
    if after != 0:
        ...
        await page.screenshot(path=str(ART / f"gyeongjo_save_c{cycle}_leftover.png"))
    return out
...
async def _run_one_cycle(browser, cycle: int, warm_state: dict | None) -> dict:
    ...
    try:
        ...
        if save_ok:
            ...
            # 상대계정 스텝 제거 후 회귀 신호 = detail 행수가 정확히 1(단건, 스트레이 빈 행 없음).
...
async def main() -> None:
    print(f"[SMOKE] 경조금신청서 실저장 사이클(F7→검증→삭제) 시작. cycles={CYCLES}. ⚠ 상신 금지·삭제 필수.", flush=True)
    ...
    try:
        for i in range(1, CYCLES + 1):
            ...
            print(f"[CYCLE {i}] {mark} run={c['run_ms']/1000:.1f}s | baseAmount={summ.get('baseAmount')} under1Year={summ.get('under1Year')} expected_supply={summ.get('expected_supply')}", flush=True)
    ...
    print("\n" + "=" * 60, flush=True)
    print(f"GYEONGJO SAVE-CYCLE SUMMARY — {passed}/{len(cycles)} PASS · avg run {avg:.1f}s · aborted={aborted}", flush=True)
    print("=" * 60, flush=True)
    ...
    (ART / "gyeongjo_smoke_cycle.json").write_text(json.dumps({"cycles": cycles, "aborted": aborted}, ensure_ascii=False, indent=1), encoding="utf-8")
    print(f"\n리포트: {ART / 'gyeongjo_smoke_cycle.json'}", flush=True)
