...
- e2e 반복 테스트는 **우리 대시보드에서 작성** → ERP 실측 검증 → 삭제(F6) → 잔존 0 확인 사이클로 한다 (ERP 에서 작성하는 e2e 는 계약 위반). 삭제 수단이 없는 화면은 비가역 동사 직전에서 멈춘다. e2e 가 실패·간헐 실패하면 다음 실행은 headed 모드로 전환해 재현한다
...
- **공유 모듈(expense_card 앞단·build_voucher_graph 백본·공용 노드) 수정 시**: 완료 보고에 영향받는 에이전트 목록 + 자동 반영 여부(공유라 즉시 반영 / 개별 수정 필요)를 **묻기 전에** 명시한다. 개별 수정이 필요하면 형제 에이전트 확산까지가 기본 스코프다
...
"수정했다/됐다"는 실측 증거와 함께만 선언한다:
- UI 변경 → 해당 화면 스크린샷(모바일 지적이면 모바일 뷰포트)으로 확인 후 보고
- 에이전트/백엔드 로직 → 관련 pytest 또는 e2e 프로브 통과 확인
- 설정 변경(LLM 모델·씽킹 등) → 적용 상태를 조회한 증거(로그·응답)를 함께 제시
- 반영에 재기동·리로드가 필요하면 그것까지 수행하고 반영 확인 후 완료 — 프론트 화면에 안 보이면 완료가 아니다
- 생성한 파일(키·문서·스크립트)은 절대경로를 완료 보고에 명시

## 실패 표면화
- 조용한 재시도 금지 — 에이전트 HITL·자동화 코드는 재시도 전 "무엇이 왜 실패했고 무엇을 바꾸는지"를 노출한다
- Claude 자신도 시도가 실패하면 원인 가설을 먼저 보고한 뒤 다음 시도를 한다. 실패를 숨긴 채 결과만 보고하지 않는다
